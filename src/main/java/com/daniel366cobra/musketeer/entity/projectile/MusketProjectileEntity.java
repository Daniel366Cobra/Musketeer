package com.daniel366cobra.musketeer.entity.projectile;
...
public class MusketProjectileEntity extends BaseProjectileEntity {
    ...
    private void spawnParticles() {
        ...
        if (life < 2) {
            for (int i = 0; i < 25; i++)
            {
                float velMagnModifier = 0.5F + random.nextFloat() - 0.5f;
        ...
        {
            this.world.addParticle(ParticleTypes.END_ROD, true, this.getParticleX(0.5), this.getBodyY(0.5), this.getParticleZ(0.5), this.random.nextFloat() * 0.2f - 0.1f, this.random.nextFloat() * 0.2f - 0.1f, this.random.nextFloat() * 0.2f - 0.1f);
            this.world.addParticle(ParticleTypes.END_ROD, true, this.getParticleX(0.5) - this.getVelocity().getX() / 2, this.getBodyY(0.5) - this.getVelocity().getY() / 2, this.getParticleZ(0.5) - this.getVelocity().getZ() / 2, this.random.nextFloat() * 0.2f - 0.1f, this.random.nextFloat() * 0.2f - 0.1f, this.random.nextFloat() * 0.2f - 0.1f);
        ...
        if (hitEntity.damage(damageSource, (float) totalDamage)) {
            ...
            if (hitEntity instanceof LivingEntity hitLivingEntity) {
                ...
                if (hitLivingEntity != shooterEntity && hitLivingEntity instanceof PlayerEntity && shooterEntity instanceof ServerPlayerEntity && !this.isSilent()) {
                    ((ServerPlayerEntity) shooterEntity).networkHandler.sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.PROJECTILE_HIT_PLAYER, 0.0f));
                }
            }
